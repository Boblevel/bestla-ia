...
test('generervideo utilise MiniMax H3 via Gradio officiel sans faux fallback statique', async () => {
  ...
  assert.match(media, /const fallback = \[prompt, null, null, this\.videoCanvas\(\), 5, 14, 42, false\]/)
  assert.match(media, /'544x960 · 9:16 fast'/)
  assert.match(media, /'960x544 · 16:9 fast'/)
  assert.match(media, /return false/)
  assert.match(media, /Math\.min\(this\.config\.mediaAi\.videoTimeoutSeconds \* 1_000, 360_000\)/)
  assert.match(media, /génération vidéo ZeroGPU a dépassé/)
  assert.match(commands, /Génération vidéo en cours… Le mode ZeroGPU rapide est utilisé/)
  ...
  assert.match(config, /MEDIA_AI_VIDEO_SPACE: z\.string\(\)\.default\('https:\/\/multimodalart-minimax-h3\.hf\.space'\)/)
  assert.match(config, /MEDIA_AI_VIDEO_MODEL: z\.string\(\)\.default\('multimodalart\/minimax-h3'\)/)
  assert.match(config, /MEDIA_AI_VIDEO_API_NAME: z\.string\(\)\.default\('\/generate'\)/)
  assert.match(config, /MEDIA_AI_HF_TOKEN/)
  assert.match(env, /MEDIA_AI_VIDEO_PROVIDER=huggingface/)
  assert.match(env, /MEDIA_AI_VIDEO_SPACE=https:\/\/multimodalart-minimax-h3\.hf\.space/)
  assert.match(env, /MEDIA_AI_VIDEO_MODEL=multimodalart\/minimax-h3/)
  assert.match(env, /MEDIA_AI_VIDEO_API_NAME=\/generate/)
})
