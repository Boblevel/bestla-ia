...
test('generervideo utilise Hugging Face ZeroGPU avec moteur principal et secours réel', async () => {
  ...
  assert.match(media, /multimodalart-minimax-h3/)
  assert.match(media, /544x960 · 9:16 fast/)
  assert.match(media, /duration: 2/)
  assert.match(media, /steps: 10/)
  assert.match(media, /lightricks-ltx-video-distilled/)
  assert.match(media, /duration_ui: 2/)
  assert.match(media, /improve_texture_flag: false/)
  ...
  assert.match(config, /MEDIA_AI_VIDEO_MODEL: z\.string\(\)\.default\('MiniMaxAI\/MiniMax-H3'\)/)
