...
test('generervideo utilise LTX Video Fast officiel sans faux fallback statique', async () => {
  ...
  assert.match(media, /private videoDimensions/)
  assert.match(media, /height: 704, width: 512/)
  assert.match(media, /height: 512, width: 704/)
  assert.match(media, /'text-to-video'/)
  assert.match(media, /const fallback: unknown\[\] = \[/)
  assert.match(media, /prompt,\s*negativePrompt,\s*null,\s*null,\s*height,\s*width,\s*'text-to-video',\s*5,\s*9,\s*42,\s*true,\s*3,\s*false,/s)
  assert.match(media, /const spaceInfo = await this\.huggingFaceSpaceInfo\(\)/)
  assert.match(media, /const endpoint = this\.resolveHuggingFaceEndpoint\(spaceInfo\)/)
  assert.match(media, /this\.buildHuggingFaceVideoPayload\(endpoint\.schema, text\)/)
  assert.match(media, /hint\.includes\('duration'\)/)
  assert.match(media, /hint\.includes\('guidance'\)/)
  assert.match(media, /hint\.includes\('improve_texture'\)/)
  ...
  assert.match(config, /LEGACY_HUGGING_FACE_VIDEO_MODELS/)
  assert.match(config, /FastVideo\/FastWan2\.2-TI2V-5B-FullAttn-Diffusers/)
  assert.match(config, /multimodalart\/minimax-h3/)
  ...
  assert.match(env, /MEDIA_AI_VIDEO_MODEL=Lightricks\/LTX-Video/)
