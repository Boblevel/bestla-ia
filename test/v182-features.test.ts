...
test('generervideo restaure Cloudflare image puis animation locale comme avant', async () => {
  ...
  assert.match(generateBlock, /const source = image \?\? await this\.generateImage\(prompt\)/)
  assert.match(generateBlock, /return this\.animateImageLocally\(source\)/)
  assert.doesNotMatch(generateBlock, /huggingFaceTextVideoRequest/)
  assert.match(media, /if \(this\.config\.mediaAi\.imageProvider === 'cloudflare'\) return this\.cloudflareImageRequest\(prompt\)/)
  assert.match(media, /private async animateImageLocally/)
