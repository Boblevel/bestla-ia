...
test('generervideo utilise Wan 2.7 via Gradio officiel sans faux fallback statique', async () => {
  ...
  assert.match(media, /lower === 'height'/)
  assert.match(media, /lower === 'width'/)
  assert.match(media, /lower === 'negative_prompt'/)
  assert.match(media, /lower === 'guidance_scale'/)
  assert.match(media, /return 4/)
  assert.match(media, /lower === 'randomize_seed'/)
  assert.match(media, /component\.includes\('image'\)/)
  assert.match(media, /component === 'textbox'/)
  assert.match(media, /parameter\?\.parameter_default/)
  assert.match(media, /component === 'slider'/)
  assert.match(media, /sliderOrdinal === 0 \? 5 : sliderOrdinal === 1 \? 4 : 0/)
  assert.match(media, /component === 'number'/)
  assert.match(media, /component === 'checkbox'/)
  assert.match(media, /parameter has no default/i)
