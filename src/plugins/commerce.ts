...
export const commerceCommands: BotCommand[] = [
  {
    name: 'produitsdigitaux',
    aliases: ['boutiquedigitale', 'cataloguedigital'],
    description: 'Affiche les produits digitaux disponibles.',
    category: 'Entreprise',
    cooldownSeconds: 5,
    async execute(ctx) {
      const products = ctx.db.listDigitalProducts(false)
      if (!products.length) return void (await ctx.reply('Aucun produit digital n’est disponible pour le moment.'))
      await ctx.reply(
        `*PRODUITS DIGITAUX*\n\n${products.map((product) => productLine(product)).join('\n\n')}\n\nPour commander : ${ctx.prefix}acheterdigital identifiant`,
      )
    },
  },
  {
    name: 'acheterdigital',
    aliases: ['commanderdigital'],
    description: 'Ouvre une demande pour acheter un produit digital.',
    usage: '<identifiant>',
    category: 'Entreprise',
    cooldownSeconds: 10,
    async execute(ctx) {
      ...
      if (!product || !product.active) return void (await ctx.reply(`Produit introuvable. Tape ${ctx.prefix}produitsdigitaux.`))
      ...
      const ticket: SupportTicket = {
        ...
        subject: `Commande digitale #${product.id} — ${product.name}`,
        status: 'ouvert',
        priority: 'haute',
        createdAt: now,
        updatedAt: now,
  ...
  {
    name: 'produitdigital',
    aliases: ['gererdigital'],
    description: 'Ajoute, active, désactive ou retire un produit digital.',
    ...
    async execute(ctx) {
      ...
      if (action === 'liste' || action === 'statut') {
        ...
        return void (await ctx.reply(products.length ? `*PRODUITS DIGITAUX*\n\n${products.map((product) => productLine(product, true)).join('\n\n')}` : 'Aucun produit digital enregistré.'))
      }
      if (action === 'ajouter') {
        ...
        if (!PRODUCT_ID.test(id) || !name || !deliveryText) {
          return void (await ctx.reply(
            `Utilisation : ${ctx.prefix}produitdigital ajouter pack1 | Pack Premium | Description publique | Lien/code/texte privé de livraison`,
          ))
        }
      ...
      await ctx.reply(`Utilisation : ${ctx.prefix}produitdigital ajouter|liste|activer|desactiver|retirer`)
    },
  },
  {
    name: 'livrerdigital',
    aliases: ['livrerproduit'],
    description: 'Livre le contenu privé d’un produit digital à un client.',
    ...
    async execute(ctx) {
      const product = ctx.db.getDigitalProduct(ctx.args[0] ?? '')
      if (!product) return void (await ctx.reply('Produit digital introuvable.'))
      const target = deliveryTarget(ctx)
      if (!target) {
        return void (await ctx.reply(`Indique le client : ${ctx.prefix}livrerdigital ${product.id} 22670000000, mentionne-le, ou réponds à son message.`))
      }
