...
async function automationsPanel(reader: Interface): Promise<void> {
  while (true) {
    ...
    if (choice === '1') await safely(reader, listSchedulesFromPanel)
      ...
      print('.produitsdigitaux')
      print('.produitdigital liste')
      print('.livrerdigital ID NUMERO')
      print(gray('Les réponses IA automatiques restent privées et les livraisons digitales exigent une action du propriétaire.'))
    })
    else if (choice === '3') await safely(reader, async () => {
