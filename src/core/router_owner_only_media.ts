...
export class MessageRouter {
  ...
  async handleMessage(runtime: SessionRuntime, message: WAMessage): Promise<void> {
    ...
    if (!parsed.isCommand && this.config.commandsEnabled && isOwner) {
    ...
    // Bestla est un assistant personnel : aucune commande n'est exécutée
    // lorsqu'elle provient d'un autre utilisateur, même si PUBLIC_MODE est activé.
    if (!isOwner) return
